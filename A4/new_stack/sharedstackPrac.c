...
void shstackpop(int stack_id)
{
    ...
    {
        int *ptr = (int *)shmat(stack_ptr->shared_stack[stack_id].shmid, NULL, 0);
        int element = *(ptr + (elem_num*data_size) - 1);
        stack_ptr->shared_stack[stack_id].elem_num--;
        printf("Stack ID = %d >> Popped element\n", stack_id);
        shmdt((void *)ptr);
    }
    shmdt((void *)stack_ptr);
}
...
void print_current_stack(int stack_id)
{
    ...
    elem_num = stack_ptr->shared_stack[stack_id].elem_num;
    st_size = stack_ptr->shared_stack[stack_id].stack_size;
